...
class stratal:
    ...
    def buildLonLatMesh(self, res=0.1, nghb=3):
        ...
        for k in range(self.curLay):
            ...
            wthi = np.divide(wthi, totperc, out=np.zeros_like(wthi), where=totperc != 0)
            coarsei = np.divide(
        ...
        del weights, denum, onIDs, zz, zi, th, xyi, thi
        if self.multilith:
            del wthi, finei, coarsei, phiSi, phiFi, phiWi
        gc.collect()
    ...
    def writeMesh(self, vtkfile="mesh", lons=None, lats=None, sigma=0.0):
        ...
        if self.multilith:
            ...
            gridToVTK(
                vtkfile, x, y, z, pointData={"dep elev": e, "th": h, "layID": t,},
            )
